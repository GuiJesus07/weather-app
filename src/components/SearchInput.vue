  ...
  searchTerm.timeout = setTimeout(async () => {
    if (searchTerm.query !== '') {
      const res = await fetch(
        `http://api.weatherapi.com/v1/search.json?key=17fe94840cf748ed8f3133246241405&q=${searchTerm.query}`
      )
...
const getWeather = async (id) => {
  const res =
    await fetch(`http://api.weatherapi.com/v1/forecast.json?key=17fe94840cf748ed8f3133246241405 &q=id:${id}&days=3&aqi=no&alerts=no
`)
  const data = await res.json()
